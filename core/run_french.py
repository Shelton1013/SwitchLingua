...
async def main():
    ...
    for i in range(0, 8000, 40):
        ...
        try:
            ...
            send_message(
                f"🔍 LANG: {config['pre_execute']['character_setting']['nationality']['first_language']} Scenario timed out after 2400 seconds: {i}"
            )
            continue
        finally:
            ...
            if results_count % 200 == 0:
                ...
                send_message(
                    f"🔍 LANG: {config['pre_execute']['character_setting']['nationality']['first_language']} Number of results finished: {results_count}"
                )
